...
def custom_attn(q, k, v, w_q, w_k, w_v, w_out):
    ###############################################################################
    # Ex.2: In this exercise, we are going to implement the most critical part of
    # the attention mechanism in CUDA.
    ###############################################################################
    ...
    # ------------------------------------------------------------------------------
    # Ex.1 S.1: Replace PyTorch standard APIs with our customized kernel to apply
    # the same operation as (Q @ K) @ V.
    # ------------------------------------------------------------------------------
    feat_q = feat_q * math.sqrt(1.0 / float(feat_q.size(-1)))  # Required
    out = custom_attn_kernel.apply(feat_q, feat_k, feat_v)
...
# Q, K, V
# Their dimension are [# token, # batch, # feature]
